...
jest.mock("./holder-index.service", () => ({
  HolderIndexService: jest.fn().mockImplementation(() => ({
    ...
    reconcileBond: jest
      .fn()
      .mockResolvedValue({ bondId: 1, holders: [], total: 0 }),
  })),
}));
...
const configProvider = {
  ...
  useValue: {
    ...
    getCreditRetirementAddress: jest
      .fn()
      .mockReturnValue("CCREDITRETIREMENTADDRESS"),
  },
};
...
describe("BondsService", () => {
  ...
  describe("getUndistributedTotal", () => {
    it("reads get_undistributed_total from the coupon engine", async () => {
      const contractService = {
        simulateCall: jest
          .fn()
          .mockResolvedValue(nativeToScVal(BigInt(42), { type: "i128" })),
      };
  ...
  describe("mature", () => {
    ...
    it("maps BondAlreadyMatured contract error to friendly BadRequestException", async () => {
      const contractService = {
        invokeContractMethod: jest
          .fn()
          .mockRejectedValue(
            new ContractException(
              "BOND_ALREADY_MATURED",
              "already matured",
              undefined,
              undefined,
              5,
  ...
  describe("buildBondResponse", () => {
    ...
    describe("getClaimableCreditDetails (itemized provenance)", () => {
      ...
      it("returns an empty itemization when the contract returns no lines", async () => {
        const contractService = {
          simulateCall: jest.fn().mockResolvedValue(nativeToScVal([])),
        };
        const moduleRef = await Test.createTestingModule({
          providers: [
            BondsService,
